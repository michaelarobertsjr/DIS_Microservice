...
def test_authentication():
    ...
    test_pass_user_data = myapp.authenticate(pass_test_token)

    assert test_pass_user_data == {'username' : 'my_test', 'email' : 'test@py.com'}

    fail_decode_test_token = pass_test_token[1:]
    test_fail_user_data = myapp.authenticate(fail_decode_test_token)
...
def test_acceptance_save_to_db():
    ...
    amt = 20
    inventory = 5000
    assert myapp.save_to_db(b_type, name, acc, price, amt, inventory) == 'Bought from stock inventory'

    amt = 50
    inventory = 40
    assert myapp.save_to_db(b_type, name, acc, price, amt, inventory) == 'Stock inventory overdrawn, inventory bought needed amt plus 100 and completed the buy'

    amt = 0
    assert myapp.save_to_db(b_type, name, acc, price, amt, inventory) == 'Invalid order amount or quoted price'

def test_get_delayed_price():

    assert type(myapp.get_delayed_price()) == type(1.0)
